...
const Text = ({ windowKey }) => {
    const { windows, isMobile } = useWindowStore();
    ...
    return (
        <div className="h-full flex flex-col bg-transparent rounded-lg overflow-hidden selection:bg-blue-500/30">
            ...
            <div className={`flex-1 overflow-y-auto p-10 text-gray-200 flex ${isMobile ? 'flex-col gap-8' : 'flex-col md:flex-row gap-12'} items-start max-w-5xl mx-auto w-full`}>
                {image && (
                    <div className={`shrink-0 ${isMobile ? 'w-full flex justify-center mb-6' : 'w-full md:w-1/3 flex justify-center sticky top-0'}`}>
                        <img
                            src={image}
                            alt={name}
                            className={`rounded-2xl shadow-2xl ${isMobile ? 'w-48 h-48' : 'w-full max-w-[250px]'} object-cover border border-white/20`}
                        />
                    </div>
                )}
